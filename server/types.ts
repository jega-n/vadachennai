...
export interface Team {
  id: TeamId;
  name: string;
  assignedBox: number | null; // 1 to 8, or null
...
export interface MatchSlotSource {
  ...
  referenceId?: string; // e.g. 'M1', 'Q1', 'SF1'
  teamId?: TeamId;
  label: string; // e.g. 'Team A', 'W(M1)', 'L(M5)', 'Q1'
}

export interface Match {
  id: string; // M1..M10, SF1, SF2, TP, FINAL
  code: string;
  name: string;
  stage: MatchStage;
  stageLabel: string;
...
export interface PublicTournamentState {
  ...
  teams: Array<Omit<Team, 'claimedByUserId'>>;
  mysteryBoxes: Array<{
    boxNumber: number;
    isLocked: boolean;
    claimed: boolean;
